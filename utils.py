...
def get_web_ext_dir():
    config = get_extension_config()
    name = config["name"]
    dir = get_comfy_dir("web/extensions/zakack")
...
def get_extension_config(reload=False):
    ...
    config_path = get_ext_dir("zakack.user.json")
    if not os.path.exists(config_path):
        config_path = get_ext_dir("zakack.json")

    if not os.path.exists(config_path):
        log("Missing zakack.json and zakack.user.json, this extension may not work correctly. Please reinstall the extension.",
...
def update_node_status(client_id, node, text, progress=None):
    ...
    PromptServer.instance.send_sync("zakack/update_status", {
        "node": node,
        "progress": progress,
        "text": text
    }, client_id)
...
async def update_node_status_async(client_id, node, text, progress=None):
    ...
    await PromptServer.instance.send("zakack/update_status", {
        "node": node,
        "progress": progress,
        "text": text
    }, client_id)
